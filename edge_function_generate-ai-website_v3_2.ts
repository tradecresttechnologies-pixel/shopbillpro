// supabase/functions/generate-ai-website/index.ts  (v3.2 — hero photo support)
//
// Adds {HERO_IMAGE_URL} to the prompt template. Fetches the shop's first
// gallery photo via sbp_get_website_generation_context() and passes it to
// fillTemplate(). Prompt v3 instructs the AI to use it as the hero
// background-image with a dark overlay (or fall back to gradient).
//
// Requires migration 054 (prompt v3 + helper RPC). Backward compatible
// with prompt v2 — the new placeholder simply isn't referenced there.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 1b. NEW (v3.2): Fetch shop generation context for hero photo URL
    let heroImageUrl = "";
    if (shopId) {
      try {
        ...
        if (!ctxResp.error && ctxResp.data?.ok) {
          ...
          // Helper RPC missing or errored — log + continue without hero photo.
          // Prompt v3 has explicit fallback for empty URL.
    ...
    // v3.2: include hero_image_url in template fill payload
    const filledPrompt = fillTemplate(tplText, { ...body, hero_image_url: heroImageUrl });

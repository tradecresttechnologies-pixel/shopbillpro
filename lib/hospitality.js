/* ══════════════════════════════════════════════════════════════════
   ShopBill Pro — Hospitality (rooms + bookings + folio) v1.0
   ...
   API:
     ...
     SBPHotel.bookings.checkIn/checkOut/cancel(shopId, bookingId, ...)
     SBPHotel.bookings.linkBill(shopId, bookingId, billId)
     SBPHotel.extras.list/add/remove(shopId, ...)
...
(function (global) {
  ...
  const bookings = {
    ...
    cancel:   (shopId, bId, reason) => _rpc('sbp_bookings_cancel',  { p_shop_id: shopId, p_booking_id: bId, p_reason: reason || null }),
  ...
  const extras = {
    ...
    remove:  (shopId, extraId)    => _rpc('sbp_booking_extras_remove', { p_shop_id: shopId, p_extra_id: extraId })
  };

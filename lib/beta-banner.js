...
(function(global) {
  ...
  function _bannerHTML(status) {
    ...
    return `
      <div id="sbp-beta-banner" style="position:relative;background:${bg};border-bottom:1px solid ${border};color:${color};padding:10px 16px;font-family:Outfit,'Noto Sans',sans-serif;font-size:13px;font-weight:600;display:flex;align-items:center;gap:12px;flex-wrap:wrap;justify-content:center;min-height:42px">
  ...
  function _ensureContainer() {
    // Prefer the page-defined placeholder (Batch 1B-A wires <div id="sbpBetaBanner">
    // immediately above the topbar in each page). Falls back to auto-creating a
    // non-sticky host at top of body if no placeholder exists.
    let host = document.getElementById('sbpBetaBanner');
    if (host) return host;

    host = document.getElementById('sbpBetaBannerHost');
    if (host) return host;
    host = document.createElement('div');
    host.id = 'sbpBetaBannerHost';
    // NOT sticky — would conflict with existing sticky topbar (z-index 50).
    // The banner scrolls with the page content; that's intentional.
    host.style.cssText = 'position:relative;left:0;right:0;z-index:49';

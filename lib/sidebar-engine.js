...
window.SBPSidebar = (function() {
  ...
  const UNIVERSAL_CORE = [
    ...
    { code: 'settings',  href: 'settings.html',  icon: '☰',  label_en: 'More',     label_hi: 'अधिक',      order: 200 },
  ];
  ...
  function _renderItem(item, layout) {
    ...
    return `<div class="${cls.join(' ')}" onclick="${onclick}" style="position:relative"><span class="ni-ic">${item.icon}</span><span class="ni-lb"><span class="lang-en">${esc(item.label_en)}</span><span class="lang-hi">${esc(item.label_hi || item.label_en)}</span></span>${badgeHtml}</div>`;
  }
  ...
  async function render(opts) {
    ...
    const containers  = (typeof opts.container === 'string')

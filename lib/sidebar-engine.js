...
window.SBPSidebar = (function() {
  ...
  function _injectStyles() {
    ...
    style.textContent =
      ...
      /* ── Canonical sidebar CSS (15-May-26) ─────────────────────────────
         Owns the sidebar's visual chrome end-to-end. Pages no longer need
         to define .dsb-* rules in their own <style> blocks. Equal specificity,
         appended late in <head>, so this wins the cascade over any per-page
         duplicate. Pages that customise via higher-specificity selectors
         (e.g. body.x .dsb-item) still win over this baseline — by design. */
      ...
      '.dsb-logo{display:flex;align-items:center;gap:10px;padding:20px 16px 14px;border-bottom:1px solid var(--bord)}' +
      '.dsb-brand{font-family:var(--font-h),system-ui,sans-serif;font-size:15px;font-weight:800;color:var(--acc)}' +
      ...
      '.dsb-item.fab-sb:hover{opacity:.88}' +

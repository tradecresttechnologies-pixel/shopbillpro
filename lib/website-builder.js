...
// ── BATCH UI-6: per-vertical storefront section spec ───────────────────
// Each biz_type maps to a concrete description of what THIS vertical's
// storefront should contain. Injected into the prompt as {VERTICAL_SPEC}
// so the AI builds a vertical-appropriate site instead of a generic one.
// Empty/missing => generic behaviour (safe: replaceAll of absent token = no-op).
const VERTICAL_SPEC = {
  food:        "This is a FOOD business (restaurant/cafe/bakery/sweets). The storefront MUST include: a prominent menu or signature-items showcase with photos and prices, opening hours, a clear 'Order on WhatsApp' / call-to-order CTA, location with map intent, and an appetising food-photo gallery. Tone: warm, appetising.",
  salon:       "This is a SALON / BEAUTY / SPA business. The storefront MUST include: a services list with prices and durations, a prominent 'Book Appointment' CTA, optional stylist/specialist highlights, before/after or work gallery, and opening hours. Tone: stylish, aspirational.",
  hospitality: "This is a HOTEL / HOSPITALITY business. The storefront MUST include: room types or accommodation options with key amenities, a clear 'Check Availability / Book Now' CTA, an amenities list, a photo gallery of rooms and property, location, and contact. Tone: welcoming, premium.",
  healthcare:  "This is a HEALTHCARE business (clinic/pharmacy/diagnostic). The storefront MUST include: services or treatments offered, practitioner or facility credentials, clinic hours, an 'Book / Enquire' CTA, location with directions intent, and clear contact. Tone: trustworthy, calm, professional. Avoid hype.",
  education:   "This is an EDUCATION business (school/tuition/coaching). The storefront MUST include: courses or programmes offered, faculty or methodology highlights, batch timings, an 'Enquire / Enrol' CTA, results or testimonials if implied, and contact. Tone: credible, encouraging.",
  services:    "This is a LOCAL SERVICES business (repair/plumbing/tailor/laundry etc.). The storefront MUST include: a clear list of services offered with indicative pricing if available, service area, a strong 'Call / WhatsApp for service' CTA, trust signals (experience, response time), and contact. Tone: dependable, no-nonsense.",
  online_brand:"This is an ONLINE / D2C BRAND. The storefront MUST include: a product or collection showcase, brand story, a shop/enquire CTA, trust signals (quality, shipping), and social proof. Tone: modern, brand-led.",
  retail:      "This is a RETAIL shop (kirana/general/garments/electronics etc.). The storefront MUST include: product categories or featured products, what makes the shop trusted locally, store hours and location, a 'Visit / WhatsApp to order' CTA, and contact. Tone: friendly, locally trusted."
};

// Resolve the spec from the free-form business_type / shop_type string.
function verticalSpecFor(businessTypeOrShopType){
  const biz = mapShopTypeToBiz(businessTypeOrShopType || '');
  return VERTICAL_SPEC[biz] || VERTICAL_SPEC['retail'] || '';
}
// Expose for the page-level payload builder.
window.SBPVerticalSpec = verticalSpecFor;

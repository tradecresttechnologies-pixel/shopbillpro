...
(function(global){
  ...
  async function open(opts){
    ...
    _state.mode = opts.mode || 'signup';   // 'signup' (default) | 'change'

    injectStyles();
    _ensureContainer();

    // ── BATCH 016: Adjust UI for change-mode vs signup-mode ──
    // In change-mode (called from settings.html), we hide the "Skip"
    // button (since this isn't initial setup) and use different copy.
    const skipBtn = document.getElementById('_sbpw_skip');
    if (skipBtn) {
      skipBtn.style.display = (_state.mode === 'change') ? 'none' : '';
    }
  ...
  async function renderStep1(){
    _state.step = 1;
    // BATCH 016: change-mode uses different copy than signup
    if (_state.mode === 'change') {
      document.getElementById('_sbpw_title').textContent = 'Change Business Type';
      document.getElementById('_sbpw_sub').textContent = 'Pick the new category for your shop. Existing data is preserved.';
    } else {
  ...
  async function renderStep2(macroCode){
    ...
    // BATCH 016: change-mode uses different copy
    if (_state.mode === 'change') {
      document.getElementById('_sbpw_title').textContent = 'Pick your new sub-type';
      document.getElementById('_sbpw_sub').textContent = 'Your sidebar modules will update to match. Existing data stays safe.';
    } else {

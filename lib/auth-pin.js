...
(function(window){
  ...
  function ensureModal(){
    ...
    const css = `
      .sbp-auth-overlay{position:fixed;inset:0;background:rgba(8,10,18,.72);backdrop-filter:blur(6px);z-index:99999;display:none;align-items:center;justify-content:center;padding:16px;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif}
      .sbp-auth-overlay.open{display:flex}
      .sbp-auth-sheet{background:var(--surf,#fff);color:var(--text,#0a0e1a);border-radius:18px;width:100%;max-width:380px;box-shadow:0 24px 60px rgba(0,0,0,.4);overflow:hidden;animation:sbpAuthSlide .25s ease-out}
      @keyframes sbpAuthSlide{from{transform:translateY(20px);opacity:0}to{transform:translateY(0);opacity:1}}

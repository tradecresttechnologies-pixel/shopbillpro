...
(function(window){
  ...
  function ensureModal(){
    ...
    overlay.innerHTML = `
      <div class="sbp-auth-sheet" role="dialog" aria-labelledby="sbp-auth-title">
        ...
        <div class="sbp-auth-bd">
          <input class="sbp-auth-pin" id="sbp-auth-pin" type="text" inputmode="numeric" pattern="[0-9]*" maxlength="12" placeholder="Enter PIN" autocomplete="one-time-code" autocorrect="off" autocapitalize="off" spellcheck="false">
    ...
    // Note: we previously had a mousedown preventDefault here to keep
    // focus on the PIN input. Removed in v5 — it was suppressing the
    // click event in Chrome's touch-emulation mode, so keypad presses
    // weren't registering. The border-color transition is also removed
    // (v3), so even if focus briefly shifts to a button and back, the
    // border-color change is instant and invisible.
  ...
  async function _submit(){
    ...
    try {
      ...
      if(res && res.ok){
        ...
        // ── Map server error codes to specific UI messages ───────────
        // Previously every non-ok was "Invalid PIN" which hid auth/setup
        // bugs (owner_id mismatch, missing shop_id, RPC errors) behind a
        // misleading message. Now each case is surfaced distinctly so
        // operators and developers can tell what's wrong.
        const errCode = (res && res.error) || 'unknown';
        const errMap = {
          'invalid_pin':         `Wrong PIN (${_attempts + 1}/${MAX_ATTEMPTS})`,
          'invalid_pin_format':  'PIN must be 4–12 digits',
          'not_owner':           '⚠️ Not the shop owner for this account. Contact admin.',
          'shop_not_found':      '⚠️ Shop record missing on server. Re-login.',
          'no_shop_id':          '⚠️ Shop ID missing — refresh the page',
          'no_supabase_client':  '⚠️ Not signed in — refresh the page',
          'rpc_error':           '⚠️ Server error — see console for details',
          'no_data':             '⚠️ Empty server response — try again'
        };
        const friendly = errMap[errCode] || ('⚠️ ' + errCode);
        // Always log full payload to console so devs can see error.message
        // and the raw server response without intervening "invalid pin" noise
        console.warn('[SBPAuth] PIN verify failed:', { errCode, response: res });

        // Only count actual wrong-PIN attempts toward the lockout. Setup
        // errors (not_owner, shop_not_found, etc.) shouldn't lock the
        // user out — they're not the user's fault.
        if(errCode === 'invalid_pin'){
          ...
          if(_attempts >= MAX_ATTEMPTS){
            ...
            errEl.textContent = `Wrong PIN (${_attempts}/${MAX_ATTEMPTS})`;
            pinInput.value = '';
            pinInput.focus();
          }
        } else {
      ...
      console.error('[SBPAuth] PIN verify threw:', e);

-- ════════════════════════════════════════════════════════════════════
-- 056_website_prompt_v3_1.sql
--
-- Prompt v3.1 — two targeted fixes over v3:
--
--   FIX 1: SECTION CONTRAST BUG
--     v3 generations produced sections with dark backgrounds but kept
--     heading text in the navy/accent color → navy-on-near-black,
--     effectively invisible. v3.1 adds a hard, explicit contrast rule
--     and forbids dark-on-dark / light-on-light combinations.
--
--   FIX 2: MISSING BOOKING BUTTONS
--     v3 generated room/service cards as plain text with no call to
--     action. The booking-form runtime (live-site.js v4.7) intercepts
--     clicks on "Book Now"/"Reserve" buttons — with no buttons, there
--     is nothing to intercept. v3.1 REQUIRES every room/service/product
--     card to end with a booking CTA button.
--
-- Everything else from v3 is preserved (hero photo, mandatory info
-- component, nav hygiene, vertical guidance, structural ids).
--
...
═══════════════════════════════════════════════════
Every card you hand-author in the business-specific section — room cards,
service cards, product cards, package cards — MUST end with a call-to-action
button. Without it customers have no way to act and the booking runtime has
...
3. <section id="about"> Business-specific section — see VERTICAL GUIDANCE.
   This is where hand-authored cards live. EVERY card here ends with a
   .card-cta button (see BOOKING CTA rule above). Light or white
   background, dark text — follow the contrast table.
...
5. <section id="services"> heading "Our Services" then
   <div data-sbp="services"></div>. White / #F8F9FA background.
...
▸ SALON / BEAUTY / SPA
  ...
  About section: "Featured services" — 3 cards, each ending in "Book Appointment"
...
▸ HOSPITALITY (hotel, guesthouse, resort, pg_hostel, homestay)
  ...
  About section: room types as 3-card grid, each card ends in "Book Now".
  Add an amenities list with emoji icons below the cards.
...
▸ HEALTHCARE (clinic, pharmacy, diagnostic, dental, physio)
  ...
  About section: specialties as cards, each ending in "Book Appointment"
...
▸ RESTAURANT / CAFE / FOOD
  ...
  About section: menu highlight cards, each ending in "Reserve a Table"
...
▸ RETAIL (kirana, grocery, general_retail, fashion)
  ...
  About section: product category cards, each ending in "Enquire Now"
...
▸ EDUCATION (coaching, tutoring, training_institute, test_prep)
  ...
  About section: course cards, each ending in "Enquire Now"
...
▸ SERVICES (plumbing, electrical, repair, cleaning, consultancy)
  ...
  About section: service cards, each ending in "Get a Quote"
...
14. CONTRAST: no dark-on-dark, no light-on-light. Follow the contrast table.
    Every section's text must be clearly readable against its background.
...
$PROMPT$,
true,                                                                              -- is_active
'v3.1 — contrast hard rule + mandatory booking CTA buttons on cards',              -- notes
'admin'
FROM next_ver
WHERE NOT EXISTS (
  SELECT 1 FROM ai_prompt_templates
  WHERE name='website_v1' AND notes LIKE 'v3.1 —%'
);


-- ── Activate v3.1, deactivate all prior versions ──────────────────
WITH new_active AS (
...
--
-- Rollback if needed:
--   UPDATE ai_prompt_templates SET is_active = (notes LIKE 'v3 —%')
--   WHERE name='website_v1';
-- ════════════════════════════════════════════════════════════════════

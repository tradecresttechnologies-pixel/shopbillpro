-- ════════════════════════════════════════════════════════════════════
-- 030a_finalize_column_fix.sql
-- Batch 022C hotfix — column reference correction (11 May 2026)
--
-- Migration 030 failed at runtime with:
--   ERROR 42703: column e.gst_amount does not exist
--   (raised inside sbp_folio_finalize_to_bill)
--
-- Root cause:
--   sbp_booking_extras (after migration 022) has these GST columns:
--     - gst_rate (numeric)
--     - taxable_amount (numeric)
--     - cgst_amount (numeric)
--     - sgst_amount (numeric)
--     - total_with_gst (numeric)
--   GST is SPLIT into cgst + sgst — there is no single `gst_amount` column.
--   My 030 referenced e.gst_amount (combined) which doesn't exist.
--
--   Also: e.unit was referenced but doesn't exist on sbp_booking_extras
--   (that field is only on bill_items).
--
-- This migration:
--   Replaces sbp_folio_finalize_to_bill with the corrected version:
--     - Combined GST per extra line = COALESCE(cgst_amount,0) + COALESCE(sgst_amount,0)
--     - If GST columns are all zero (legacy rows from before 022),
--       compute GST from category + amount using the same rules as
--       lib/hospitality.js extraGstForCategory()
--     - Removes e.unit reference (column doesn't exist)
--
-- Idempotent. Safe to re-run.
-- Prerequisites: 030 must have been attempted (creates the function we're replacing).
-- ════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- Room line + GST slab (post 22 Sep 2025 reform)
  ...
  --
  -- For each extra:
  --   taxable = COALESCE(e.taxable_amount, e.amount)
  --   gst_per_line = COALESCE(cgst_amount + sgst_amount,
  --                           taxable * (category_gst_rate / 100))
  --
  -- The COALESCE fallback covers legacy extras where the GST columns
  -- never got populated (entries created before migration 022's
  -- trigger that auto-computes them).
  -- ──────────────────────────────────────────────────────────────
  SELECT
    COALESCE(SUM(COALESCE(e.taxable_amount, e.amount, 0)), 0),
    COALESCE(SUM(
      CASE
        -- Use stored GST split if present
        WHEN COALESCE(e.cgst_amount, 0) + COALESCE(e.sgst_amount, 0) > 0
          THEN COALESCE(e.cgst_amount, 0) + COALESCE(e.sgst_amount, 0)
        -- Otherwise compute from gst_rate × taxable
        WHEN COALESCE(e.gst_rate, 0) > 0
          THEN COALESCE(e.taxable_amount, e.amount, 0) * COALESCE(e.gst_rate, 0) / 100
        -- Last resort: derive rate from category
  ...
  -- Paid amount from unified ledger
  ...
  -- Each extra as its own bill_items line
  FOR v_extra IN
  ...
  LOOP
    ...
    BEGIN
      ...
      -- Combined GST: split columns if present, else derive
      ...
      ELSIF v_line_gst = 0 THEN
        -- Derive from category as last resort
        v_eff_gst_pct := CASE
...
-- ──────────────── End of 030a_finalize_column_fix.sql ────────────

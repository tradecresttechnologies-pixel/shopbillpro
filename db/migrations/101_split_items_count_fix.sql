-- ════════════════════════════════════════════════════════════════════
-- 101_split_items_count_fix.sql
-- ════════════════════════════════════════════════════════════════════
-- WHAT
--   Hotfix for migration 100. Three split RPCs were inserting into a
--   non-existent bills.items_count column.
--
--   Live error: Split failed: column "items_count" of relation "bills"
--               does not exist
--
-- WHY
--   bills.items_count was assumed to exist by the author of mig 100.
--   It never has — item counts are computed in-line via
--     (SELECT COUNT(*) FROM bill_items WHERE bill_id = b.id)
--   in customer-history and similar RPCs. No consumer reads a stored
--   items_count column, so no schema change is needed; we just stop
--   writing the bogus column.
--
-- ACTION
--   Re-creates these three split RPCs with items_count refs removed:
--     • sbp_ro_split_equal
--     • sbp_ro_split_custom
...
--   3. Retry Split Bill — should succeed.
--
-- ROLLBACK
--   Re-run migration 100 as-is (reintroduces the bug). Don't.
-- ════════════════════════════════════════════════════════════════════

...
//
// v3.9 (this file): home_multipage prompt switch for Business multi-page shops
//
// CHANGES vs v3.7:
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // v3.9: when generating the home page as part of a multi-page sequence,
    // use the 'home_multipage' prompt template (path-based nav, teaser content)
    // instead of the standard 'home' prompt (anchor-based nav, full content).
    const isMultipageHome = pageSlug === "home" && body.is_multipage === true;
    const promptPageSlug = isMultipageHome ? "home_multipage" : pageSlug;
    ...
    //   - v3.8: page-aware via page_slug
    //   - v3.9: home of a multi-page sequence uses 'home_multipage' template
    ...
    console.log(`[v3.9] plan=${planNorm} page=${pageSlug} prompt=${promptPageSlug} multipage=${isMultipageHome} resolved_provider=${resolvedProvider} final=${providerUsed}`);
